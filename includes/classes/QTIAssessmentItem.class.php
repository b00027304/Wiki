...
abstract class QTIAssessmentItem {
	...
	public function showForm($data = null) {
		...
		include $wgScriptPath . "/includes/include/htmlheader.php";
		?>
			...
			<form id="edititem" action="<?php $action = new EditAssessmentItemAction(); echo $action->actionURL(); ?>" method="post">
				<dl>
					...
					<dd><textarea rows="4" cols="20" name="description" id="description"><?php if (isset($this->data["description"])) echo htmlspecialchars($this->data["description"]); ?></textarea></dd>
					...
					<dd><textarea class="qtitinymce resizable" rows="4" cols="64" name="stimulus" id="stimulus"><?php if (isset($this->data["stimulus"])) echo htmlspecialchars($this->data["stimulus"]); ?></textarea></dd>
		...
		<?php
		include $wgScriptPath . "/includes/include/htmlfooter.php";
	}
