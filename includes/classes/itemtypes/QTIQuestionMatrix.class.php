			...
			<table id="questions">
				...
				for ($i = 0; array_key_exists("question_{$i}_prompt", $this->data); $i++) { $odd = $i % 2; ?>
					<tr class="question row<?php echo $odd; ?>" id="question_<?php echo $i; ?>">
						<td><textarea class="prompt" rows="2" cols="20" name="question_<?php echo $i; ?>_prompt" id="question_<?php echo $i; ?>_prompt"><?php if (isset($this->data["question_{$i}_prompt"])) echo htmlspecialchars($this->data["question_{$i}_prompt"]); ?></textarea></td>
						<td class="answer">
