...
class Article extends Page {
	...
	public function view() {
		...
		global $wgUseFileCache, $wgUseETag,$IP;
		

		//$myFile = "E:/Documents and Settings/Toshiba/Desktop/Sondous.txt";
		//$fh = fopen($myFile, 'w') or die("can't open file");
		
		wfProfileIn( __METHOD__ );

		if ($this->getTitle()-> isSpecialPage() || $this->getTitle()->isMainPage() || $this-> getTitle()->getNsText()=='Category' 
			...
			{$this->view2();
			 ...
			 $Colonpos=strrpos($this->mPage->getRawText(),':');
			 $SecArgpos=strrpos($this->mPage->getRawText(),']');
			 $catxt="[[Category:".trim(substr($this->mPage->getRawText(),$Colonpos+1,$SecArgpos-$Colonpos-2))."]]";
			 $trans = array("$catxt" => " ");
			 $outputxml=trim(strtr($this->mPage->getRawText(),$trans));
			 
			/* if (simplexml_load_string($outputxml)==true && $this-> getTitle()->getNsText()!='Category' )
			 { $wgOut->addHTML($proc->transformToXML(simplexml_load_string($outputxml)));
			 
			 # Parse just to get categories, displaytitle, etc.
			 $this->mParserOutput = $wgParser->parse( $this->getContent(), $this->getTitle(), $parserOptions );
	...
	public function doViewParse() {
		...
		//$myFile = "E:/Documents and Settings/Toshiba/Desktop/moi.txt";
		//$fh = fopen($myFile, 'w') or die("can't open file");
		...
		$trans = array("$catxt" => " ");
		$outputxml=trim(strtr($this->getContent(),$trans));
		
		
   		
		if (simplexml_load_string($outputxml)==true )
		{
			$wgOut->addHTML($proc->transformToXML(simplexml_load_string($outputxml)));
