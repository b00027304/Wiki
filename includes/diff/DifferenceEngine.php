...
CONTROL;
	...
	function renderNewRevision() {
		global $wgOut, $wgUser,$IP,$wgParser;
		...
		if ( wfRunHooks( 'ArticleContentOnDiff', array( $this, $wgOut ) ) ) {
			...
			//$myFile = "E:/Documents and Settings/Toshiba/Desktop/7ayati.txt";
			...
			if ( $this->mTitle->isCssJsSubpage() || $this->mTitle->isCssOrJsPage() ) {
				...
				if( $pOutput ) {
					...
					if($this->getTitle()->getNsText()=='' &&simplexml_load_string($this->StripCategoryFromXml($this->mNewRev->getRawText()))==true )
					{
					
						$wgOut->addParserOutputNoText( $pOutput );
								
						$NewRev=$this->StripCategoryFromXml($this->mNewRev->getRawText());
					
						$wgOut->addHTML($proc->transformToXML(simplexml_load_string($NewRev)));
				...
				if($this->getTitle()->getNsText()=='' &&simplexml_load_string($this->StripCategoryFromXml($this->mNewRev->getRawText()))==true )
				{
					 ...
					 $parserOptions = Wikipage::factory($this->mTitle)-> getParserOptions() ;					
					 $wgOut->addParserOutputNoText( $wgParser->parse( $this->mNewRev->getRawText(),$this->mTitle, $parserOptions ));
	...
	public function getDiffBody() {
		...
		if($this->getTitle()->getNsText()=='' && !$this->getTitle()->isMainPage() &&
		   simplexml_load_string($this->StripCategoryFromXml($this->mOldtext))==true && 
		   simplexml_load_string($this->StripCategoryFromXml($this->mNewtext))==true )
		{
	        $oldtext=trim($proc->transformToXML(simplexml_load_string($this->StripCategoryFromXml($this->mOldtext))));
			$newtext=trim($proc->transformToXML(simplexml_load_string($this->StripCategoryFromXml($this->mNewtext))));
			
			$OldCat=$this->GetCategoryFromXml($this->mOldtext);
			$NewCat=$this->GetCategoryFromXml($this->mNewtext);
					
			$oldtext.="\n".$OldCat;
			$newtext.="\n".$NewCat;
						
		   $difftext = $this->generateDiffBody( $oldtext, $newtext );
		}
		else 	
