<?php
/**
 * Implements Special:AddQuestion
 *
...
/**
 * implements Special:AddQuestion - Run title search if the page doesn't exist creats it
 * @ingroup SpecialPage
 */
/*For GPL V2.
...
class SpecialAddQuestion extends SpecialPage {
	/**
	...
	global $wgRequest, $wgUser, $wgOut;
		...
		$wgOut->addModules('Cat_List');
		
		//$myFile = "E:/Documents and Settings/Toshiba/Desktop/sms.txt";
		...
		// Fetch the search term (title of the question added by the user)
		
		$search = str_replace( "\n", " ", $wgRequest->getText( 'CategoryList', $titleParam ) );
		
		
		if($wgRequest->getText('CategoryList','')!='' || $wgRequest->getText('CategoryListBox','')!='')
		{
		if($wgRequest->getText('CategoryList','')!='')	
		$search=$this->generateQtitle(trim($wgRequest->getText('CategoryList')));
		
		//if($wgRequest->getText('CategoryListBox','')!='')
		//$search=$this->generateQtitle(str_replace( '_', ' ',trim($wgRequest->getText('CategoryListBox'))));
		...
		//check if other pages with the same exact tile exists
	...
	public function goResult( $term ) {
		...
		if( is_null( $t ) ) 
		{
			...
			$wgOut->addHtml( Xml::openElement('form',array('name'=>'AddQSp','method'=>'get','action' => $wgScript)));
			$wgOut->addHTML("<input type='hidden' name=\"title\" value=\"Special:AddQuestion\"/>");
			$wgOut->addHtml( "<label>Choose A Category </label></br>");
			$wgOut->addHtml($this->addCategorylist());
			$wgOut->addHTML("</br> <input type=\"submit\" name=\"AddQues\" value=\"AddQuestion\" id=\"mw-AddQuesButton\" >");
			$wgOut->addHTML( '</form>' );
			return;
	   }
		...
		if( !is_null( $t ) ) {
			...
			if( $wgGoToEdit ) {
				...
				if( $wgGoToEdit ) {
					...
					$URL=$t->getFullURL( array( 'action' => 'edit' ));
									
					if($wgRequest->getVal('CategoryList','')!='' )
					$URL.="&CategoryList=".$wgRequest->getVal('CategoryList');
					
					//if($wgRequest->getVal('CategoryListBox','')!='' )
					//$URL.="&CategoryList=".$wgRequest->getVal('CategoryListBox');
	...
	public function showResults( $term ) {
		...
		if( $textMatches && $textMatches->hasSuggestion() ) {
			$st = SpecialPage::getTitleFor( 'AddQuestion' );

			# mirror Go/Search behaviour of original request ..
			$didYouMeanParams = array( 'AddQuestion' => $textMatches->getSuggestionQuery() );
	...
	protected function generateQtitle($Catxt){
		
		$db = wfGetDB( DB_SLAVE );
		$category_table = $db->tableName( 'category' );
		$db_result = $db->query( "select cat_pages FROM ${category_table} where trim(cat_title)='${Catxt}'" );
		
		$row = $db->fetchObject( $db_result );
		
		if($row->cat_pages==null || empty($row->cat_pages))
		  $Qtitle=str_replace('_',' ',trim($Catxt).".Q1");
		  else
		  { $count=(string)$row->cat_pages+1;
		   $Qtitle=str_replace('_',' ',trim($Catxt).".Q".trim($count));}
...
protected function addCategorylist(){
		...
		$listBox=" <select size=\"1\" name=\"CategoryList\" id=\"mw_CategorylistBox\" style=\"width:500px\">";
