...
abstract class BaseTemplate extends QuickTemplate {
	...
	function getSidebar( $options = array() ) {
		...
		if ( !isset( $sidebar['ADDINGQUESTION'] ) ) {
			$sidebar['ADDINGQUESTION'] = true;
		}
		...
		if ( !isset( $options['addingquestion'] ) || $options['addingquestion'] !== true ) {
			unset( $sidebar['ADDINGQUESTION'] );
		}
...
function makeAddingQInput( $attrs = array() ) {
		$realAttrs = array(
				'type' => 'text',
				'name' => 'AddQuestion',
				'value' => isset( $this->data['addingquestion'] ) ? $this->data['addingquestion'] : '',
		);
	...
	function makeAddQButton( $attrs = array() ) {
