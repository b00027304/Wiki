...
class MultipleUpload extends SpecialUpload {
	...
	protected function loadRequest($request) {
		...
		//$myFile = "E:/Documents and Settings/Toshiba/Desktop/Fairouz.txt";
		...
		if ( !$fromSession ) {
			for ( $i = 0; $i < MultipleUpload::getMaxUploadFiles(); $i++ ) {
			    ...
			    $newpage=$this->generateQtitle(str_replace( '_', ' ',trim($request->getText('wpDestFile'.$i))));

		      	$t = Title::newFromText( $newpage );
				while( $t->exists())
				{ $newpage=$this->generateQtitle(str_replace( '_', ' ',trim($request->getText('wpDestFile'.$i))));
				  $t = Title::newFromText( $newpage );
				}
               ...
               if( !is_null( $t ) ) 
               {
	               	...
	               	$XmlFile = $_FILES['wpUploadFile' . $i]['tmp_name'];
	               	
	               	if(!empty($XmlFile)) {
		               	
		               	$fhandle = fopen($XmlFile, 'r') or die("can't open file");
		               	$QtiItem=fread($fhandle, filesize($XmlFile));
		               	fclose($fhandle);
		               
		               	//add  Category
		               	$QtiItem.="[[Category:".$request->getVal('wpDestFile'.$i)."]]";
		
		               	$article = new Article($t);
		               	
		               	$res = $article->doEdit($QtiItem, '',0,false,$wgUser);
		...
		if ($flag)
			$wgOut->addWikiText($sucessful.' pages were created sucessfully.');
		else 
			$wgOut->addWikiText($sucessful.' pages were created sucessfully. and '.$failure.' pages failed to be created');
	...
	protected function getUploadForm( $message = '', $sessionKeys = array(), $hideIgnoreWarning = false ) {
		...
		$msg="Use the form below to upload XML files. Each XML file represents a Question. You have to choose the category (Learning Outcome)
		under which the question would be list. You can upload a maximum of 20 XML files at a time. ";
		
		# Add text to form
		$form->addPreText( '<div id="uploadtext">'."<p>{$msg}</p>".'</div>' );
	...
	protected function generateQtitle($Catxt){
		...
		if($db->numRows($db_result)==0)
			$Qtitle=trim($Catxt)."Q1";
		else
		{ $count=(string)$row->cat_pages+1;
		$Qtitle=trim($Catxt).".Q".trim($count);
		}
...
class MultiUploadForm extends UploadForm {
	...
	public function __construct( $options = array(), $context = null, $messagePrefix = 'multiupload' ) {
		...
		/*$wgOut->addHTML('<div id="uploadtext">'."<p>{$msg}</p>".'</div>' );
		
		# Add Category List to form
		$wgOut->addHTML($this->getCatList());
		*/
	...
	protected function getCatList(){
		...
		{ $options[str_replace('_',' ',trim($row->page_title))]=strtr(trim($row->page_title),'_',' ');}
	...
	protected function addUploadJS() {
		...
		//$out->addScriptFile( '$wgScriptPath/extensions/MultiUpload/edit.js' );
